{
    description = "A flake providing a custom service";
    ...
    flake-utils.lib.eachDefaultSystem (system:
        ...
        in {
        # Define custom packages and services here
        ...
        nixosModules.custom-service = { lib, config, pkgs, nvidiaCache, ... }: {
            ...
            config = lib.mkIf config.services.myCustomService.enable {
                systemd.services.myCustomService = {
                    ...
                    script = ''
                    ${pkgs.python3}/bin/python -c 'import time; with open("${config.services.myCustomService.path}", "a") as f: f.write(f"Service run at {time.ctime()}\n")'
                    '';
                };
            };
        };
